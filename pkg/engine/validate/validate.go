...
var arrayOfCombineOriginalPatternMap []interface{}

func getOriginalPatternKey(origPattern interface{}, key string) {
...
func combineMap(v interface{}) {
	...
	for i := l; i >= 0; i-- {
		...
		if i == l {
			tempArray := [1]interface{}{v}
			temp[patternKey[i]] = tempArray
	...
	arrayOfCombineOriginalPatternMap = append(arrayOfCombineOriginalPatternMap, combineOriginalPatternMap)
}
...
func validateMap(log logr.Logger, resourceMap, patternMap map[string]interface{}, origPattern interface{}, path string, ac *common.AnchorKey) (string, error) {
	...
	for key, patternElement := range anchors {
		if commonAnchors.IsExistenceAnchor(key) {
			...
			getOriginalPatternKey(typedOrigPattern, key)
			fmt.Println("----arrayOfCombineOriginalPatternMap: ", arrayOfCombineOriginalPatternMap)
		} else {
			tempArray := [1]interface{}{origPattern}
			arrayOfCombineOriginalPatternMap = nil
			arrayOfCombineOriginalPatternMap = append(arrayOfCombineOriginalPatternMap, tempArray)
		}
